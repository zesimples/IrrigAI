...
Uses weather data from WeatherContext. Latitude comes from the farm record
(user-configured). Falls back to Hargreaves when solar or wind data is missing.
"""
...
def penman_monteith(w: DailyWeather, lat_deg: float) -> float | None:
    ...
    Rso = (0.75 + 2e-5 * 0) * Ra  # assume elevation ≈ 0; good enough for MVP
...
def compute_et0(w: DailyWeather, lat_deg: float) -> tuple[float | None, str]:
