...
- **Production uses BOTH compose files** — `docker compose -f docker-compose.yml -f docker-compose.prod.yml …` for every prod command. `docker-compose.prod.yml` resets the dev `ports` (`ports: !reset []`) on Postgres/Redis/backend/frontend so **only nginx publishes host ports**; verify the merged config with `docker compose -f docker-compose.yml -f docker-compose.prod.yml config`. Step-by-step runbooks live in `docs/runbooks/` (`deploy.md`, `restore-backup.md`, `rollback.md`, `rotate-secrets.md`, `provider-outage.md`). Dev hot-reload mounts come from `docker-compose.dev.yml` (`make dev`); a stack started with plain `docker compose up` bakes code at build time for backend AND frontend — rebuild to pick up new code.
