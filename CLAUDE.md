...
**Dose do dia (dose-centric recommendation):** the headline is a 4-band dose (`reforcada`/`normal`/`curta`/`pode_saltar` — `engine/dose_presentation.py`), not a binary verdict; dosage is computed on **every** pipeline run (reserve days get a reduced dose). Dose source precedence (mirrors `swc_source`, stored in `inputs_snapshot`): `configured` (application rate/emitters) > `probe_learned` (per-sector `irrigation_fingerprint` learned weekly from probe-detected irrigation events, 25-day window/staleness, ≥3 usable events, VWC-only) > `mm_only`. `RecommendationAction` (irrigate/skip/defer) is unchanged underneath — alerts and AI guards still key off it. The Step 11.6 fingerprint lookup is **savepoint-isolated** (`pipeline._load_sector_fingerprint`) — a DB error there (e.g. the `irrigation_fingerprint` migration lagging a deploy) degrades the dose to `mm_only` and increments `dose_fingerprint_lookup_failures_total` instead of 500-ing the whole recommendation. **A soil/CC-PMP edit regenerates the recommendation:** `PUT /sectors/{id}/crop-profile` calls `generate_recommendation` when a soil-bounds field (`field_capacity`/`wilting_point`/`soil_preset_id`) changes, best-effort (rolls back + returns the saved profile on failure), so depletion isn't frozen at the last stored rec until the next scheduler run.
