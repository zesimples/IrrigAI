...
**Done in the 2026-07-03 cycle (branch `feat/innoliva-per-plot-weather`, dev-verified, NOT yet merged / not on prod):** Innoliva onboarding **remodelled** to ONE farm + a plot per polo (superseding the earlier 6-farm design), 77 VWC-probe olive sectors; and **per-plot weather** (Plot weather config, `plot_id` on weather tables, per-call adapter project/device, per-plot ingestion, `build_weather_context` plot resolution with farm + representative-plot fallback) — see **One farm, plots spanning several MyIrrigation projects** above. Onboarding also sets a representative farm location and skips observation fetch for device-less plots (Conceição forecast-only).

**Open, roughly prioritized:**
0. **Innoliva prod rollout** — merge `feat/innoliva-per-plot-weather`, then on prod (order matters, migration lives in the image): `git pull` → `docker compose up -d --build backend worker` → `docker compose exec backend alembic upgrade head` → copy `docs/innoliva_device_mapping.csv` into the container → run `onboard_innoliva.py` (real `INNOLIVA_*` env + stable `ENCRYPTION_KEY`) → verify 1 farm / 6 plots (each with its project+device) / 77 sectors + per-plot weather (Conceição forecast-only) + existing farms' weather unchanged → watch one scheduler cycle for the 406 rate. Also fix the pre-existing `auto_calibration.analyze_sector` `"moisture"`-vs-`"soil_moisture"` mismatch (calibration preview returns null for VWC sectors; `/run` works).
