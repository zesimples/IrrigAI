...
**Rootzone chart overlay + why "Soma" can mislead:** depletion is computed from the **root-zone-weighted** SWC (`probe_interpreter._compute_rootzone`, sensors with `depth_cm <= effective_root_depth`), but the probe chart's "Soma" view sums **all** depths — so a dry topsoil under a wet subsoil (deep sensors below the root zone) reads 82% depletion while Soma looks green. Both are correct; they measure different things. The chart's "Profundidades" view overlays the engine's rootzone-weighted SWC line (`probe_interpreter.weighted_rootzone_series`, served by `api/v1/probes`) so it visibly agrees with the recommendation; the line's timestamps are built from the already-downsampled per-depth points so they can't phase-drift under an interval. Effective root depth (young/mature by tree age, then phenological-stage override) is resolved by the shared `pipeline.resolve_effective_root_depth_m` so the chart and engine never diverge (same idiom as `resolve_sector_soil_bounds`). When no sensor is in the root zone, both the engine and the overlay fall back to weighting all depths. The real lever when a dry-rootzone call looks wrong is the **crop profile's stage root depth** (agronomic), not code.
